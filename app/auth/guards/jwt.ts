...
export interface JwtUserProviderContract<RealUser> {
   ...
   */
  findById(identifier: string | number | BigInt): Promise<JwtGuardUser<RealUser>>
}
...
export class JwtGuard<UserProvider extends JwtUserProviderContract<unknown>>
  ...
  async generate(user: UserProvider[typeof symbols.PROVIDER_REAL_USER]) {
    ...
    const userModel: any = providerUser.getOriginal()
    const payload = {
   ...
   */
    ...
    var getTokenCookie = async () => {
      const token = this.#ctx.request.cookie("access_token");
      if (!token) {
        throw new errors.E_UNAUTHORIZED_ACCESS('Unauthorized access', {
          guardDriverName: this.driverName,
        })
    ...
    try {
      var token = await getTokenHeader();
    } catch {
